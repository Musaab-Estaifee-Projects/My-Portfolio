...
const MusicPlayer: React.FC = () => {
  ...
  const [isMuted, setIsMuted] = useState(false); // Start muted to allow autoplay
  ...
  const fadeVolume = (targetVolume: number, duration: number) => {
    if (audioRef.current) {
      const step = (targetVolume - audioRef.current.volume) / (duration / 50);
      const interval = setInterval(() => {
        if (audioRef.current) {
          const newVolume = Math.min(
            Math.max(audioRef.current.volume + step, 0),
            1
          );
          audioRef.current.volume = newVolume;

          if (Math.abs(newVolume - targetVolume) < Math.abs(step)) {
  ...
  const togglePlay = () => {
    if (audioRef.current) {
      if (isPlaying) {
        fadeVolume(0, 500); // Gradually decrease volume before pausing
        setTimeout(() => {
        ...
        audioRef.current.play().then(() => {
          fadeVolume(volume, 500); // Gradually increase volume when resuming
          setIsPlaying(true);
        });
      }
    }
  ...
  const toggleMute = () => {
    if (audioRef.current) {
      if (isMuted) {
        fadeVolume(volume, 500); // Gradually increase volume to set level
        audioRef.current.muted = false;
      } else {
        fadeVolume(0, 500); // Gradually decrease volume before muting
        setTimeout(() => {
  ...
  const handleVolumeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const newVolume = Math.min(Math.max(parseFloat(e.target.value), 0), 1); // Clamp volume
    setVolume(newVolume);
    if (audioRef.current) {
      audioRef.current.volume = newVolume;
    }
  ...
  // Automatically play the music when the component is mounted
  useEffect(() => {
    audioRef.current = new Audio("/sounds/mozart-40.mp3");
    audioRef.current.loop = true; // Loop the symphony indefinitely
    audioRef.current.volume = volume; // Set initial volume
    audioRef.current.muted = isMuted; // Start muted for autoplay
    // audioRef.current.play().catch((error) => {
    //   console.error("Error playing audio:", error);
    // });
    audioRef.current.play().catch(() => {});

    setIsPlaying(true); // Set state to true since it should autoplay

    // Listen for any user interaction to unmute and start playing if blocked
    ...
    // Add listeners to catch user interaction for autoplay
  ...
  return (
    <>
      ...
      <div className="fixed top-3 right-3 max-sm:top-1 max-sm:-right-1 flex flex-col items-center gap-2 z-[9999]">
